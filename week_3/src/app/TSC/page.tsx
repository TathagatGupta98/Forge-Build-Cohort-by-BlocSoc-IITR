...
export default function MUSDPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl border border-indigo-50">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-xl font-bold text-slate-800">Tathagat Stable Coin Vault</h1>
            ...
            <div className="relative space-y-2">
              <div className="relative rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <span className="ml-2 font-bold text-slate-500">{mode === "mint" ? "ETH" : "TSC"}</span>
                </div>
              </div>
              ...
              <div className="relative rounded-2xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <span className={`ml-2 font-bold ${mode === "mint" ? "text-indigo-600" : "text-orange-600"}`}>{mode === "mint" ? "TSC" : "ETH"}</span>
                </div>
              </div>
            </div>

            <button onClick={handleTransaction} disabled={!inputValue || isLoading} className={`w-full flex items-center justify-center rounded-xl py-4 text-lg font-bold text-white shadow-lg transition active:scale-[0.98] ${mode === "mint" ? "bg-indigo-600 hover:bg-indigo-700" : "bg-orange-500 hover:bg-orange-600"} ${(isLoading || !inputValue) ? "opacity-50 cursor-not-allowed" : ""}`}>
